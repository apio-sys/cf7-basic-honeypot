 ...
 * Text Domain: contact-form-7-simple-honeypot
...
function cf7_simple_honeypot_cf7_missing_notice() {
    ...
    <div class="notice notice-error">
        <p><?php _e('Contact Form 7 Simple Honeypot requires Contact Form 7 to be installed and activated.', 'contact-form-7-simple-honeypot'); ?></p>
    </div>
    <?php
}
...
function cf7_simple_honeypot_add_admin_menu() {
    add_submenu_page(
        'wpcf7',
        __('Simple Honeypot', 'contact-form-7-simple-honeypot'),
        __('Simple Honeypot', 'contact-form-7-simple-honeypot'),
        'manage_options',
        'contact-form-7-simple-honeypot',
...
function cf7_simple_honeypot_settings_page() {
    ...
    <div class="wrap">
        ...
        <div class="notice notice-info">
            <h3><?php _e('How to Use', 'contact-form-7-simple-honeypot'); ?></h3>
            <p><?php _e('Add the following shortcodes to your Contact Form 7 forms:', 'contact-form-7-simple-honeypot'); ?></p>
            <p><code>[honeypot]</code> - <?php _e('Adds the hidden honeypot field', 'contact-form-7-simple-honeypot'); ?></p>
            <p><code>[timestamp]</code> - <?php _e('Adds time-based validation', 'contact-form-7-simple-honeypot'); ?></p>
            <p><strong><?php _e('Example form:', 'contact-form-7-simple-honeypot'); ?></strong></p>
...
[submit "Send"]</pre>
        ...
        <form method="post" action="options.php">
            ...
            <table class="form-table">
                <tr>
                    <th colspan="2"><h2><?php _e('Honeypot Settings', 'contact-form-7-simple-honeypot'); ?></h2></th>
                </tr>
                <tr>
                    <th scope="row">
                        <label for="honeypot_field_name"><?php _e('Honeypot Field Name', 'contact-form-7-simple-honeypot'); ?></label>
                    </th>
                    <td>
                        ...
                        <p class="description"><?php _e('The name of the hidden field. Use CF7-style names (e.g., your-website, your-company)', 'contact-form-7-simple-honeypot'); ?></p>
                    </td>
                </tr>
                <tr>
                    <th colspan="2"><h2><?php _e('Time-Based Validation', 'contact-form-7-simple-honeypot'); ?></h2></th>
                </tr>
                <tr>
                    <th scope="row">
                        <label for="min_submit_time"><?php _e('Minimum Submit Time (seconds)', 'contact-form-7-simple-honeypot'); ?></label>
                    </th>
                    <td>
                        ...
                        <p class="description"><?php _e('Forms submitted faster than this will be marked as spam (recommended: 3-5 seconds)', 'contact-form-7-simple-honeypot'); ?></p>
                    </td>
                </tr>
                <tr>
                    <th scope="row">
                        <label for="max_submit_time"><?php _e('Maximum Submit Time (seconds)', 'contact-form-7-simple-honeypot'); ?></label>
                    </th>
                    <td>
                        ...
                        <p class="description"><?php _e('Forms older than this will be marked as spam (recommended: 3600 = 1 hour)', 'contact-form-7-simple-honeypot'); ?></p>
                    </td>
                </tr>
                <tr>
                    <th colspan="2"><h2><?php _e('Content Analysis Settings', 'contact-form-7-simple-honeypot'); ?></h2></th>
                </tr>
                <tr>
                    <th scope="row">
                        <label for="message_field_names"><?php _e('Message Field Names', 'contact-form-7-simple-honeypot'); ?></label>
                    </th>
                    <td>
                        ...
                        <p class="description"><?php _e('One field name per line. These are the fields that will be analyzed for spam content.', 'contact-form-7-simple-honeypot'); ?></p>
                    </td>
                </tr>
                <tr>
                    <th scope="row">
                        <label for="max_urls"><?php _e('Maximum URLs Allowed', 'contact-form-7-simple-honeypot'); ?></label>
                    </th>
                    <td>
                        ...
                        <p class="description"><?php _e('Messages with more URLs than this will be marked as spam', 'contact-form-7-simple-honeypot'); ?></p>
                    </td>
                </tr>
                <tr>
                    <th scope="row">
                        <label for="max_caps_percentage"><?php _e('Maximum Uppercase Percentage', 'contact-form-7-simple-honeypot'); ?></label>
                    </th>
                    <td>
                        ...
                        <p class="description"><?php _e('Messages with more uppercase letters than this percentage will be marked as spam', 'contact-form-7-simple-honeypot'); ?></p>
                    </td>
                </tr>
                <tr>
                    <th scope="row">
                        <label for="min_words"><?php _e('Minimum Word Count', 'contact-form-7-simple-honeypot'); ?></label>
                    </th>
                    <td>
                        ...
                        <p class="description"><?php _e('Messages with fewer words than this will be marked as spam', 'contact-form-7-simple-honeypot'); ?></p>
                    </td>
                </tr>
                <tr>
                    <th scope="row">
                        <label for="spam_keywords"><?php _e('Spam Keywords', 'contact-form-7-simple-honeypot'); ?></label>
                    </th>
                    <td>
                        ...
                        <p class="description"><?php _e('One keyword or phrase per line. Messages containing any of these will be marked as spam. Case-insensitive.', 'contact-form-7-simple-honeypot'); ?></p>
                    </td>
                </tr>
            </table>
            <?php submit_button(); ?>
...
function cf7_simple_honeypot_validation($spam, $submission) {
    ...
    if (isset($data[$field_name]) && !empty($data[$field_name])) {
        ...
        $submission->add_spam_log(array(
            'agent' => 'honeypot',
            'reason' => __('Honeypot field was filled', 'contact-form-7-simple-honeypot')
        ));
    }
    return $spam;
}
...
function cf7_simple_honeypot_timestamp_validation($spam, $submission) {
    ...
    if (!isset($data['cf7_timestamp'])) {
        ...
        $submission->add_spam_log(array(
            'agent' => 'timestamp',
            'reason' => __('Timestamp field missing', 'contact-form-7-simple-honeypot')
        ));
        return $spam;
    }
    ...
    if ($time_elapsed < $min_time) {
        ...
        $submission->add_spam_log(array(
            'agent' => 'timestamp',
            'reason' => sprintf(__('Form submitted too quickly (%d seconds)', 'contact-form-7-simple-honeypot'), $time_elapsed)
        ));
        return $spam;
    }
    // Form took too long
    if ($time_elapsed > $max_time) {
        ...
        $submission->add_spam_log(array(
            'agent' => 'timestamp',
            'reason' => sprintf(__('Form session expired (%d seconds old)', 'contact-form-7-simple-honeypot'), $time_elapsed)
        ));
        return $spam;
    }
    return $spam;
...
function cf7_simple_honeypot_content_analysis($spam, $submission) {
    ...
    if ($url_count > $max_urls) {
        ...
        $submission->add_spam_log(array(
            ...
            'reason' => sprintf(__('Too many URLs in message (%d found, max %d allowed)', 'contact-form-7-simple-honeypot'), $url_count, $max_urls)
        ));
        return $spam;
    }
    ...
    if (strlen($letters_only) > 10) {
        ...
        if ($caps_percentage > $max_caps) {
            ...
            $submission->add_spam_log(array(
                'agent' => 'content-analysis',
                'reason' => sprintf(__('Excessive uppercase text (%.0f%% caps, max %d%% allowed)', 'contact-form-7-simple-honeypot'), $caps_percentage, $max_caps)
            ));
            return $spam;
        }
    }
    ...
    if ($word_count < $min_words) {
        ...
        $submission->add_spam_log(array(
            'agent' => 'content-analysis',
            'reason' => sprintf(__('Message too short (%d words, min %d required)', 'contact-form-7-simple-honeypot'), $word_count, $min_words)
        ));
        return $spam;
    }
    ...
    foreach ($spam_keywords as $keyword) {
        if (strpos($message_lower, strtolower($keyword)) !== false) {
            ...
            $submission->add_spam_log(array(
                'agent' => 'content-analysis',
                'reason' => sprintf(__('Spam keyword detected: "%s"', 'contact-form-7-simple-honeypot'), $keyword)
            ));
            return $spam;
        }
    }
    ...
    if (preg_match('/(.)\1{5,}/', $message) || preg_match('/(.{2,})\1{3,}/', $message)) {
        ...
        $submission->add_spam_log(array(
            'agent' => 'content-analysis',
            'reason' => __('Repetitive text pattern detected', 'contact-form-7-simple-honeypot')
        ));
        return $spam;
    }
    ...
    if ($total_chars > 0) {
        ...
        if ($special_char_percentage > 30) {
            ...
            $submission->add_spam_log(array(
                'agent' => 'content-analysis',
                'reason' => sprintf(__('Excessive special characters (%.0f%% of message)', 'contact-form-7-simple-honeypot'), $special_char_percentage)
            ));
            return $spam;
        }
    }
